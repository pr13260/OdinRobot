# cherry-picked from ayraHikari
from . import SYS_ADMIN, dispatcher, OWNER_ID

Owner = OWNER_ID
NoResUser = [OWNER_ID, 777000, SYS_ADMIN]
AntiSpamValue = 25
				...
				if int(time) >= int(value['restrict']):
					...
					if value['status']:
						...
						if status == False:
							...
							dispatcher.bot.sendMessage(Owner, "⚠ Alert: user `{}` was detected spam.".format(user_id), parse_mode="markdown")
		...
		if check_user['status_hard']:
			...
			try:
				if getbotinfo.status in ('administrator', 'creator'):
					...
					dispatcher.bot.sendMessage(chat_id, "I've banned this spammer!", reply_to_message_id=message.message_id)
					return True
			except:
				pass
			if message.chat.type != 'private':
				dispatcher.bot.sendMessage(chat_id, "Flood was detected!\nI'm out, invite me again when admins stop flooding.\n\nThanks.")
				dispatcher.bot.leaveChat(chat_id)
				return True
		dispatcher.bot.sendMessage(chat_id, "Alert! This user triggered my Antispam algorithm and will be restricted for a while.\n\nIf this continues I will ban and create a spam report for this user!", reply_to_message_id=message.message_id)
		return True
